...
audioData *fileToBuffer(char *fileName) {
	...
	newAudioFile->buffer = (float *)malloc(sizeof(float)*length);
	sf_readf_float(file, newAudioFile->buffer, length);
	sf_close(file);
...
//-----------------------------------------------------------------------------
void writeWavFile(float *audio, int sample_rate,
					int numChannels, int numFrames, int numOutChannels, char *outFileName)
{
	int i;
...
audioData *zeroPadToNextPowerOfTwo(audioData *audio) {
	...
	int newLength = calculateNextPowerOfTwo(audio->numChannels*audio->numFrames);
	// Create new buffer
	...
	for (i=0; i<audio->numChannels*audio->numFrames; i++) {
		newBuffer[i] = audio->buffer[i];
	}
	// free the old buffer
	free(audio->buffer);
	// add the new buffer to the audioData struct
	audio->buffer = newBuffer;
	// calculate new numFrames variable
	audio->numFrames = newLength/audio->numChannels;
...
//		printf("signal: mono\nimpulse: mono\n");
		float *outputBuffer;
		// Convolve
		int length = convolve(signal->buffer, impulse->buffer, signal->numFrames, impulse->numFrames, &outputBuffer);
		// For normalizing later on
		for (i=0; i<signal->numFrames; i++) {
			if (fabs(signal->buffer[i]) > max) {
				max = fabs(signal->buffer[i]);
			}
		}
		// Normalize signal buffer, multiply by dry/wet coefficient
		for (i=0; i<signal->numFrames; i++) {
			signal->buffer[i] *= (1-dry_wet)/max;
		}
		// Add dry signal to output buffer
		for (i=0; i<signal->numFrames; i++) {
			outputBuffer[i] += signal->buffer[i];
		}
...
//		printf("signal: stereo\nimpulse: mono\n");
		...
		float *signal_left = (float *)malloc(sizeof(float)*signal->numFrames);
		// Create buffer for right channel of signal
		float *signal_right = (float *)malloc(sizeof(float)*signal->numFrames);
		// Copy signal data into buffers
		for (i=0; i<signal->numFrames; i++) {
			signal_left[i] = signal->buffer[2*i];
			signal_right[i] = signal->buffer[2*i+1];
			// For normalizing later on
			if (fabs(signal->buffer[2*i]) > max) {
				max = fabs(signal->buffer[2*i]);
			}
			if (fabs(signal->buffer[2*i+1]) > max) {
				max = fabs(signal->buffer[2*i+1]);
			}
		}
		// Convolve
		int length = convolve(signal_left, impulse->buffer, signal->numFrames, impulse->numFrames, &outLeft);
		convolve(signal_right, impulse->buffer, signal->numFrames, impulse->numFrames, &outRight);
		...
		// Add dry signal to output buffer
		for (i=0; i<signal->numFrames; i++) {
			outputBuffer[2*i] += signal->buffer[2*i];
			outputBuffer[2*i+1] += signal->buffer[2*i+1];
		}
		// Normalize output buffer again
		max = 0;
...
//		printf("signal: mono\nimpulse: stereo\n");
		...
		float *impulse_left = (float *)malloc(sizeof(float)*impulse->numFrames);
		// Create buffer for right channel of impulse
		float *impulse_right = (float *)malloc(sizeof(float)*impulse->numFrames);
		// Copy impulse data into buffers
		for (i=0; i<impulse->numFrames; i++) {
			impulse_left[i] = impulse->buffer[2*i];
			impulse_right[i] = impulse->buffer[2*i+1];
		}
		// For normalizing later on
		for (i=0; i<signal->numFrames; i++) {
			if (fabs(signal->buffer[i]) > max) {
		...
		// Convolve
		int length = convolve(signal->buffer, impulse_left, signal->numFrames, impulse->numFrames, &outLeft);
		convolve(signal->buffer, impulse_right, signal->numFrames, impulse->numFrames, &outRight);
		// Recombine left and right channels
		float *outputBuffer = (float *)malloc(sizeof(float)*length*2);
		for (i=0; i<length; i++) {
			outputBuffer[2*i] = outLeft[i]*dry_wet;
		...
		// Add dry signal to output buffer
		for (i=0; i<signal->numFrames; i++) {
			outputBuffer[2*i] += signal->buffer[i];
			outputBuffer[2*i+1] += signal->buffer[i];
		}
		// Normalize output buffer again
		max = 0;
		for (i=0; i<length*2; i++) {
...
//		printf("signal: stereo\nimpulse: stereo\n");
		...
		// Convolve
		int length = convolve(signal_left, impulse_left, signal->numFrames, impulse->numFrames, &outLeft);
		convolve(signal_right, impulse_right, signal->numFrames, impulse->numFrames, &outRight);
		// Recombine left and right channels
		float *outputBuffer = (float *)malloc(sizeof(float)*length*2);
		for (i=0; i<length; i++) {
			outputBuffer[2*i] = outLeft[i]*dry_wet;
	...
	if (signal->numChannels == 2 && impulse->numChannels == 2) {
		...
		for (j=0; j<signal->numFrames; j++) {
			for (k=0; k<impulse->numFrames; k++) {
				outputBuffer[2*(j+k)] += signal->buffer[2*j]*impulse->buffer[2*k];
				outputBuffer[2*(j+k)+1] += signal->buffer[2*j+1]*impulse->buffer[2*k+1];
			}
		}
		...
		for (i=0; i<signal->numFrames*2; i++) {
			outputBuffer[i] += signal->buffer[i]*(1-dry_wet);
		}
		// Normalize
